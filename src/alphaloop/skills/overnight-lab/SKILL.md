...
3. Submit with the morning page YAML box at `/` **or** `alphaloop submit --spec PATH`; both return `run_id` immediately. The packaged morning page can POST YAML to `/v1/jobs`, polls progress every two seconds, and cannot change hard gates.
