...
3. If the spec names a local parquet other than the packaged example, cache it with `alphaloop dataset PATH` (prints `dataset_id` / `sha256`; does not create a job). **Preview** with `alphaloop preview --spec PATH` (does not create a job). Then freeze with the morning page YAML box at `/` **or** `alphaloop submit --spec PATH`; both return `run_id` immediately. The packaged morning page can POST YAML to `/v1/jobs`, polls progress every two seconds, and cannot change hard gates.
