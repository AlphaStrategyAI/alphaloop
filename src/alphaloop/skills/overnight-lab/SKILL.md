...
4. **Poll** the morning page or `alphaloop status` (latest job) or `alphaloop status RUN_ID`. Parse JSON with `alphaloop status --json` or `alphaloop status RUN_ID --json`. Do not block a chat, CLI, or MCP tool call for hours. There is no overnight MCP session.
5. Open the morning console at `http://127.0.0.1:8765/` or inspect artifacts under the run directory. `alphaloop replay RUN_ID` rewrites `report.md` from sealed artifacts and prints the same five-minute verdict as `status`. Parse JSON with `alphaloop replay RUN_ID --json`. It does not re-run gates.
