...
4. **Poll** the morning page or `alphaloop status RUN_ID`. Parse JSON with `alphaloop status RUN_ID --json`. Do not block a chat, CLI, or MCP tool call for hours. There is no overnight MCP session.
