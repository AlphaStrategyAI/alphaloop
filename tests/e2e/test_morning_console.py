...
def test_home_shows_promise_and_submit_form(real_daemon, browser_page):
    ...
    page.wait_for_function(
        """() => {
          const el = document.getElementById('preview-protocol');
          if (!el) return false;
    ...
    if outcome == "FOUND":
        ...
        page.wait_for_function(
            """() => {
              ...
              return node && (node.textContent || '').indexOf('FOUND') === 0;
            }""",
            timeout=10000,
        )
        receipt = page.locator("#verdict #export-status").inner_text()
        ...
        assert page.locator("#handoff button.export-asb").count() == 0
